...
namespace SWD392.OutfitBox.API.Controllers
{
    ...
    public class BrandController : ControllerBase
    {
        ...
        [HttpGet("brands/featured-brands")]
        public async Task<ActionResult<BaseResponse<List<BrandModel>>>> GetFeaturedBrands()
        {
            BaseResponse<List<BrandModel>> response;
            try
            {
                var data = await _brandService.GetFeaturedBrands();
                response = new BaseResponse<List<BrandModel>>("Get brands successfully.", System.Net.HttpStatusCode.OK, data);
            }
            catch (Exception ex)
            {
